...
{
    public class PlLegalTextParser : LegalTextParser
    {
        public PlLegalTextParser(Tbx tbxSource) : base(tbxSource)
        {
        }

        private static readonly Regex m_sectionHeading = new Regex(@"^(Art\.\s?(?<ArtNum>([0-9]*))\.?)$", RegexOptions.IgnoreCase | RegexOptions.Compiled | RegexOptions.ExplicitCapture);
